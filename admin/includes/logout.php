<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if(isset($_POST['logout'])){
            ...
            modal_msg(array("Successfully Logout ({$username})"), "Logout Message", "{$serverAddress}/signin.php");
            //printf("<meta http-equiv=\"refresh\" content=\"0; url='http://%s/index.php'\" />", $_SERVER['HTTP_HOST']);
            $_SESSION = array();
            session_destroy();
        } else {
            modal_msg(array("Illegal Direct Access"), "Error", "{$serverAddress}/index.php");
        }
    } else {
        modal_msg(array("Illegal Direct Access"), "Error", "{$serverAddress}/index.php");
    }
